...
rule_file = os.path.join("rules", "fraud_rule.json")

# input data
df = (
    pd.read_csv("fraud_data.csv").sample(10000).reset_index(drop=True)
...
fraud_mod = Model(
    xgb_fraud, "fraud_risk", data, mode="classification", preprocess=preprocess
)
